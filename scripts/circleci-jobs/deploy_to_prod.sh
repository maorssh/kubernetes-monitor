...
PIPELINE_ID = $(curl -s --fail --show-error \
  ...
  https://circleci.com/api/v2/project/github/snyk/$KUBERNETES_MONITOR_DEPLOYER_REPO/pipeline/mine | jq -r .items[0].id)
...
if [[ ${APPROVAL} == "Accepted." ]]; then
  echo "Approved Production deployment"
fi
